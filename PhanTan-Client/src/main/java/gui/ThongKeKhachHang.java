...
public class ThongKeKhachHang extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        tb_KH.setModel(new DefaultTableModel(
            new Object [][] {
    ...
    public void getDSKH() throws Exception {
        ...
        for (KhachHang kh : arr) {
            i++;
            int soHoaDon = hd_dao.getSoHoaDonTheoKhachHang(kh.getMaKhachHang());
            double doanhThu = hd_dao.getDoanhThuTheoKhachHang(kh.getMaKhachHang());
    ...
    private void filterByRevenueRange(String range) throws Exception {
        ...
        try {
            ...
            switch (range) {
                case "<200 000":
                    havingClause = "HAVING COALESCE(SUM(h.tongTien), 0) <= 200000";
                    break;
                case "200 000 - 500 000":
                    havingClause = "HAVING SUM(h.tongTien) > 200000 AND SUM(h.tongTien) <= 500000";
                    break;
                case "500 000 - 1 000 000":
                    havingClause = "HAVING SUM(h.tongTien) > 500000 AND SUM(h.tongTien) <= 1000000";
                    break;
                case ">1 000 000":
                    havingClause = "HAVING SUM(h.tongTien) > 1000000";
                    break;
                default:
                    throw new IllegalArgumentException("Khoảng doanh thu không hợp lệ: " + range);
            }

            // Ghi log để debug
            System.out.println("Query condition: " + havingClause);

            // Gọi DAO để lấy danh sách khách hàng
            arr = kh_dao.getTKKhachHangDoanhThu(havingClause);
            System.out.println("Số kết quả: " + arr.size());
            ...
            e.printStackTrace();
            throw new Exception("Lỗi lọc theo khoảng doanh thu: " + e.getMessage());
        }
    }
