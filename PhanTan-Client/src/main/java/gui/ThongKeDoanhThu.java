...
<<<<<<< HEAD:src/main/java/gui/ThongKeDoanhThu.java
...
=======
>>>>>>> fc5b56ce17db639ce812d5f739a9c3507403ad3f:PhanTan-Client/src/main/java/gui/ThongKeDoanhThu.java

    public void NgayTheoDoanhThu() throws Exception {
        Date selectedDate = jDateChooser1.getDate();

        if (selectedDate == null) {
            Notifications.getInstance().show(Notifications.Type.WARNING, "Vui lòng chọn ngày!");
            Logger.getLogger(ThongKeDoanhThu.class.getName()).warning("Chưa chọn ngày trong jDateChooser1");
            return;
        }
        ...
        try {
            ...
            Logger.getLogger(ThongKeDoanhThu.class.getName()).info(
                    "Số lượng bản ghi doanh thu theo ngày: " + arrThang.size() + " cho tháng " + month + "/" + year);
            ...
            DecimalFormat df = new DecimalFormat("#,##0");
            ThangVaDoanhThu highestDay = null;
            ThangVaDoanhThu lowestDay = null;
            double maxRevenue = Double.MIN_VALUE;
            double minRevenue = Double.MAX_VALUE;
            double price = 0;

            int rowIndex = 1;
            for (ThangVaDoanhThu dayRevenue : arrThang) {
                Object[] row = {
                        rowIndex++,
                        dayRevenue.getThang() + "/" + month + "/" + year,
                        df.format(dayRevenue.getDoanhThu()) + " VND",
                        hd.getSizeHoaDonTheoNgay(dayRevenue.getThang(), month, year),
                        hd.getSoLuongKhachHangNgay(dayRevenue.getThang(), month, year)
                };
                // Gán price cho ngày được chọn
                if (dayRevenue.getThang() == day) {
                    price = dayRevenue.getDoanhThu();
                }
                model.addRow(row);

                chart1.addData(new ModelChart(String.valueOf(dayRevenue.getThang()), new double[]{dayRevenue.getDoanhThu()}));

                if (dayRevenue.getDoanhThu() > maxRevenue) {
                    maxRevenue = dayRevenue.getDoanhThu();
                    highestDay = dayRevenue;
                }
                if (dayRevenue.getDoanhThu() < minRevenue && dayRevenue.getDoanhThu() > 0) {
                    minRevenue = dayRevenue.getDoanhThu();
                    lowestDay = dayRevenue;
                }
            }
            ...
            // Cập nhật nhãn cao nhất/thấp nhất
            if (highestDay != null) {
                lbCaoNhat.setText("Ngày " + highestDay.getThang() + ": " + df.format(highestDay.getDoanhThu()) + " VND");
            } else {
                lbCaoNhat.setText("Không có dữ liệu");
            }
            if (lowestDay != null) {
                lbThapNhat.setText("Ngày " + lowestDay.getThang() + ": " + df.format(lowestDay.getDoanhThu()) + " VND");
            } else {
                lbThapNhat.setText("Không có dữ liệu");
            }

            // Cập nhật thẻ
            int soKh = hd.getSoLuongKhachHangNgay(day, month, year);
            int countHoaDon = hd.getSizeHoaDonTheoNgay(day, month, year);
            double doanhThuNam = hd.getDoanhThuCuaNam(year);
            double chiTieu = doanhThuNam > 0 ? (price / doanhThuNam) * 100 : 0;
            double chiTieuHoaDon = hd.getSizeOfMonth(month, year) > 0 ? (countHoaDon * 1.0 / hd.getSizeOfMonth(month, year)) * 100 : 0;
            double chiTieuKH = hd.getSoLuongKhachHangThang(month, year) > 0 ? (soKh * 1.0 / hd.getSoLuongKhachHangThang(month, year)) * 100 : 0;
            ...
            Logger.getLogger(ThongKeDoanhThu.class.getName()).log(Level.SEVERE,
                    "Lỗi khi lọc doanh thu theo ngày: " + e.getMessage(), e);
            Notifications.getInstance().show(Notifications.Type.ERROR,
                    "Lỗi khi tải dữ liệu doanh thu theo ngày: " + e.getMessage());
            throw e;
        }
    }
